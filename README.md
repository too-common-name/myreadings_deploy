...
- `.env.docker/.env.local`: Environment files to separate configurations for Docker execution and local development.
...
```bash
docker-compose -f myreadings_deploy/docker-compose.yaml --env-file myreadings_deploy/.env.docker --profile dev up
```
...
```bash
docker-compose -f myreadings_deploy/docker-compose.yaml --env-file myreadings_deploy/.env.docker --profile dev --profile app up --build
```
